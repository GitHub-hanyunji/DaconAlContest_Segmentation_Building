...
from model.attention_backbone__eca_bottleneck_aspp__cbam__nonlocal_6 import get_model

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = get_model(num_classes=1, pretrained=False).to(device)
    
    checkpoint_path = './output_last/ckpt/checkpoint_149.pth'
    ...
    submit.to_csv('./deeplabv3_last_01.csv', index=False)
